...
**No frameworks.** The agent loop is an explicit `while` loop that sends messages to the Claude API, dispatches tool calls, and feeds results back. This makes the control flow readable and debuggable, and you can step through every iteration.
...
- **Multi-source search** that queries both OpenAlex (200M+ works with citation data) and arXiv (preprints) in parallel
- **Conversational narrowing** that refines vague queries into structured search profiles before searching
- **Citation-aware ranking** with citation counts pulled from OpenAlex
- **"Find similar" flow** to select a paper from results and search for related work, with automatic deduplication
- **Zotero integration** to save papers directly to your Zotero library (v7+ via Connector API, v10+ via local API with collection support)
- **Eval suite** with gold-standard retrieval evals and recall@K scoring (10 cases spanning ML, neuroscience, crypto, NLP)
- **Animated terminal UI** built with Rich tables and TerminalTextEffects
...
├── agent.py        # ReAct loop with Claude tool use
...
| Academic data | OpenAlex + arXiv | Free, comprehensive, complementary coverage |
